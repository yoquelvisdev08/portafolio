...
const Header = () => {
  ...
  const nameGradientPrimary = isBlue
    ? 'bg-gradient-to-r from-white to-white/60 bg-clip-text text-transparent'
    : 'bg-gradient-to-r from-on-surface to-on-surface-variant bg-clip-text text-transparent';

  const nameGradientSecondary = isBlue
    ? 'bg-gradient-to-r from-white/90 to-white/50 bg-clip-text text-transparent'
    : 'bg-gradient-to-r from-on-surface/90 to-secondary bg-clip-text text-transparent';
  ...
  return (
    ...
    >
      ...
      <div className="page-x relative z-10 mx-auto grid w-full max-w-container-max grid-cols-1 items-start gap-8 sm:gap-10 lg:grid-cols-2 lg:gap-8">
        ...
        >
          <motion.div
            className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-full border border-primary-fixed/30 bg-primary-fixed/10 px-3 py-2 font-label-caps text-xs text-primary-fixed shadow-[0_0_15px_rgba(195,244,0,0.15)] sm:px-4 sm:text-label-caps"
          ...
          <motion.div variants={shouldReduceMotion ? reducedMotionVariant : revealItem}>
            ...
            >
              ...
              <br />
              <span className={nameGradientSecondary}>{nameParts.slice(1).join(' ')}</span>
            </h1>
          </motion.div>
        ...
        <motion.div
          className="order-1 flex w-full min-w-0 justify-center lg:order-2 lg:w-auto lg:justify-end lg:pt-[6.5rem]"
          variants={shouldReduceMotion ? reducedMotionVariant : revealItem}
        >
          <HeroPhoto />
        </motion.div>
